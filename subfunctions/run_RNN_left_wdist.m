function [ rp_vec, r_in_s ] = run_RNN_left_wdist( p, r_in )
...
%    init_conds = des_out_left(:,1) + noise_sigma.*randn(N,1);
    init_conds = mean([mean(des_out_left(:,1:20),2), mean(des_out_right(:,1:20),2)],2) + noise_sigma*randn(N,1);       
...
for i=1:N_trials_distr
    ...
    if i<=N_trials_distr/2
        inp_dist_temp = smooth(inp_dist_vec.*(1-dist_sigma*abs(randn)),fr_smooth);
    else 
        inp_dist_temp = smooth(inp_dist_vec.*(1+dist_sigma*abs(randn)),fr_smooth);
    end
